...
def predictions(classifier, test_data):
    # input is the test_data for demonstration purposes
    # should handle raw data preprocessing in real life scenario and different input data types (e.g. database, json, etc.)
    """
    ...
    # Drop ID columns
    X_test = test_data.drop(columns=["has_done_upselling", "customer_id", "rating_account_id"], errors="ignore")
    y_true = test_data["has_done_upselling"]

    # Predict probabilities
    y_pred_proba = classifier.predict_proba(X_test)[:, 1]
    y_pred = (y_pred_proba >= 0.5).astype(int)

## this part takes a while to run, so only run if needed, otherwise comment out
# takes about 3 minutes in code space
# shap model could be precomputed and saved for faster inference in real life scenario
# additionally direction of impact (positive/negative) could be added for better interpretability
    ...
    predictions_df = pd.DataFrame({
        "customer_id": test_data.get("customer_id", pd.Series(range(len(y_true)))),
        "true_label": y_true,
